...
#### Claude Code 增强 Profile：ECC / Superpowers 可以加入开发链路

项目已经集成 Claude Code，因此 ECC / Superpowers 这类能力最适合落在 Developer Agent，而不是全平台强依赖。

正确定位：

- Claude Code 是 Developer Agent 的代码执行器。
- ECC / Superpowers / gstack 是 Developer Agent 的工程增强 Profile。
- Agent Hub 自己仍然负责 Orchestrator、Artifact Contract、Evidence Gate、模型路由和阶段流转。

不建议做法：

- 不要把整个 Agent Hub 绑定成 Claude Code 插件壳。
- 不要全量污染用户级 `~/.claude` 配置。
- 不要让 Product / Designer / Architect / QA / DevOps / Acceptance 都依赖 Claude Code 才能工作。

建议集成方式：

```json
{
  "developer_profile": "claude-code-enhanced",
  "executor_backend": "claude_code",
  "mounted_packs": [
    "openspec",
    "superpowers",
    "gstack",
    "everything-claude-code-lite"
  ],
  "scope": "task_worktree",
  "mode": "temporary_project_profile"
}
```

##### 必须吸收的能力

- ECC 的多 Agent 分工：引入 reviewer、build-error-resolver、security-reviewer、database-reviewer 等开发子代理。
- ECC 的 Skill 库思想：按语言、框架、任务类型加载 TDD、Debugging、Code Review、Refactor、Build Fix 技能。
- ECC 的 MCP 思想：Developer 默认绑定 GitHub、filesystem、commands、browser、context7。
- ECC 的 Memory 思想：开发失败、构建错误、修复策略写入 task memory 和 learning loop。
- Superpowers 的流程约束：写代码前查技能，写代码中持续校验，写完后验证。
- gstack 的交付约束：写完代码后必须进入 `/qa`、`/ship`、`/retro`。

##### 按需挂载的能力

- 语言规则：TypeScript / Python / Go / Java 等按项目技术栈挂载。
- 框架规则：Vue / FastAPI / Next.js / Django 等按仓库识别挂载。
- 安全扫描：涉及 auth、支付、凭证、外部 API 时强制挂载。
- 数据库审查：涉及 migration、schema、SQL 查询时强制挂载。
- 性能审查：涉及大循环、缓存、并发、IO、前端性能时强制挂载。

##### Developer Agent 内部执行顺序

```text
/code
  -> 加载 spec.md、file_plan.json、acceptance_criteria.json
  -> 挂载 claude-code-enhanced profile
  -> OpenSpec 检查：需求是否冻结
  -> Superpowers 检查：匹配技能和规则
  -> Claude Code 写真实文件
  -> after_write hook：format / lint / typecheck
  -> build-error-resolver 修复构建错误
  -> security / performance / framework reviewer 按需审查
...
Developer Agent 可以“站在 Claude Code 肩膀上”，但不能把平台命运交给 Claude Code。

也就是说：

- Claude Code 负责写代码。
- ECC / Superpowers 负责提升开发质量。
- gstack 负责把代码推向交付。
- Agent Hub 负责把这一切产品化、可审计、可重试、可验收。
