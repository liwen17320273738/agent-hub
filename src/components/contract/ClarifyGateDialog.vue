  ...
  >
    ...
    <div v-if="currentStep === 1" class="clarify-body">
      ...
      <div class="clarify-actions">
        ...
        <el-button type="primary" @click="currentStep = 2" :disabled="draftMetrics.length === 0">
          {{ t('contract.next') }}
        </el-button>
      </div>
    </div>
    ...
    <div v-if="currentStep === 2" class="clarify-body">
      ...
      <div class="contract-summary">
        ...
        <div class="summary-section">
          ...
          <ul>
            <li v-for="m in draftMetrics" :key="m.name">
              <strong>{{ m.name }}</strong>:
              {{ m.direction === 'increase' ? '≥' : m.direction === 'decrease' ? '≤' : '=' }}{{ m.target_value }}
              ({{ m.source }}, {{ m.measurement_window_days || 30 }}{{ t('contract.days') }})
            </li>
          </ul>
        </div>
        <div class="summary-section">
...
async function handleSign() {
  ...
  try {
    ...
    // Step 2: create outcome contract
    ...
    ElMessage.error(e.message || t('contract.signError'))
  } finally {
    submitLoading.value = false
  }
}
